...
async function getStatus(req, res) {
    ...
    if (user && user.state) {
        ...
        const waterTarget = (user.state.profile && parseInt(user.state.profile.waterTarget)) || 2500;
        const waterEntries = Array.isArray(user.state.water?.[today]) ? user.state.water[today] : [];
        const waterTotal = waterEntries.reduce((s, e) => s + (parseInt(e.ml) || 0), 0);
        if (waterTotal < waterTarget) {
            missing.push(`Вода (випив ${waterTotal} з ${waterTarget} мл, залишилось ${waterTarget - waterTotal})`);
        }

        const vacuum = Array.isArray(habits.vacuum) ? habits.vacuum : [0, 0, 0, 0, 0];
        if (vacuum.length < 5 || vacuum.some(v => !v)) {
...
async function getQuote(req, res) {
    try {
        const total = Array.isArray(stoicQuotes) && stoicQuotes.length ? stoicQuotes.length : 1;
        const dayIndex = Math.floor(Date.now() / 86400000) % total;
        const selected = stoicQuotes[dayIndex] || {};
        res.json({
            ok: true,
            quote: selected.quote || 'Перешкода на шляху стає шляхом.',
            author: selected.author || 'Марк Аврелій'
        });
    } catch(err) {
        res.json({
            ok: true,
